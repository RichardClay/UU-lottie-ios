...
///加载zip文件
- (instancetype)initWithAnimationFromJsonZipUrl:(NSString *)url complete:(void (^)(bool result))unzipLoadSuccess{
    self = [self initWithFrame:CGRectZero];
    NSURL   *downLoadUrl = [NSURL URLWithString:url];
    if (self) {
        LOTComposition *laScene = [LOTComposition animationWithFilePath:[LOTAnimationView getTargetCachePathCacheUrl:url]];
        if (laScene) {
          laScene.cacheKey = downLoadUrl.absoluteString;
          [self _initializeAnimationContainer];
          [self _setupWithSceneModel:laScene];
         if (unzipLoadSuccess) {
             unzipLoadSuccess(true);
         }
        }else{
            NSString *md5        = [LOTAnimationView md5:url];///将下载链接转为md5 后面当做文件夹的名字
                    ...
                    if (!error) {
                        ...
                        if(!Error){
                            ...
                            if([zip UnzipOpenFile:zipPath]){
                                ...
                                LOTComposition *laScene = [LOTComposition animationWithFilePath:[LOTAnimationView getTargetCachePathCacheUrl:url]];
                                dispatch_async(dispatch_get_main_queue(), ^(void) {
                                  [[LOTAnimationCache sharedCache] addAnimation:laScene forKey:downLoadUrl.absoluteString];
                                  laScene.cacheKey = downLoadUrl.absoluteString;
                                  [self _initializeAnimationContainer];
                                  [self _setupWithSceneModel:laScene];
                                  if (unzipLoadSuccess) {
...
+ (NSString *)getTargetCachePathCacheUrl:(NSString *)cacheUrl{
    NSString *md5        = [LOTAnimationView md5:cacheUrl];
    NSArray  *pathes     = NSSearchPathForDirectoriesInDomains(NSCachesDirectory,NSUserDomainMask,YES);
    NSString *path       = [pathes objectAtIndex:0];
    NSString *finishPath = [NSString stringWithFormat:@"%@/zipDownload/%@",path,md5];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSDirectoryEnumerator *myDirectoryEnumerator = [fileManager enumeratorAtPath:finishPath];
    NSString *file;
    NSString *result;
    while((file = [myDirectoryEnumerator nextObject])) {
        if([[file pathExtension] isEqualToString:@"json"]){
            {
                result = [finishPath stringByAppendingPathComponent:file];
            }
        }
    }
    return result;
